...
public class SpringAopProxyFactory implements io.r2dbc.proxy.callback.ProxyFactory {
	...
	private static class ProxyInterceptor implements MethodInterceptor {
		...
		public Object invoke(MethodInvocation methodInvocation) throws Throwable {
			return this.callbackHandler.invoke("", methodInvocation.getMethod(), methodInvocation.getArguments());
		}
	}

	private ProxyFactory createSpringProxyFactory(ProxyInterceptor interceptor, Class<?>... proxyInterfaces) {
	...
	public ConnectionFactory wrapConnectionFactory(ConnectionFactory connectionFactory) {
		...
		ProxyFactory proxyFactory = createSpringProxyFactory(new ProxyInterceptor(handler), ConnectionFactory.class);
	...
	public Connection wrapConnection(Connection connection, ConnectionInfo connectionInfo) {
		...
		ProxyFactory proxyFactory = createSpringProxyFactory(new ProxyInterceptor(handler), Connection.class);
	...
	public Batch wrapBatch(Batch batch, ConnectionInfo connectionInfo) {
		...
		ProxyFactory proxyFactory = createSpringProxyFactory(new ProxyInterceptor(handler), Batch.class);
	...
	public Statement wrapStatement(Statement statement, String query, ConnectionInfo connectionInfo) {
		...
		ProxyFactory proxyFactory = createSpringProxyFactory(new ProxyInterceptor(handler), Statement.class);
	...
	public Result wrapResult(Result result, QueryExecutionInfo queryExecutionInfo) {
		...
		ProxyFactory proxyFactory = createSpringProxyFactory(new ProxyInterceptor(handler), Result.class);
